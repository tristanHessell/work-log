...
function App(): JSX.Element {
  ...
  const [currentItem, setCurrentItem] = useState<Partial<TravelItem>>({id: uuid(), startingOdometer: 0});
  ...
  function onClickDeleteItem (e: any, item: TravelItem): void {
    ...
    setTravelItems([...travelItems.slice(0, itemIndex), ...travelItems.slice(itemIndex + 1)]);
  }
...
async function onChangeOdometer (e: any, item: Partial<TravelItem>) : Promise<void> {
  setCurrentItem({
    ...item,
...
async function onClickStartPlace (e: any, item: Partial<TravelItem>) : Promise<void> {
...
async function onClickEndPlace (e: any, item: Partial<TravelItem>) : Promise<void> {
...
useEffect(() => {
  ...
  return (
    <div className="App">
      ...
      <button onClick={(e) => onClickAddNew(e, currentItem)} disabled={!currentItem.start || !currentItem.end}>(Save and) Start new</button>
      <div>
        <input ref={startingOdoRef} type="number" value={currentItem.startingOdometer} placeholder="Starting odometer reading" onChange={(e) => onChangeOdometer(e, currentItem)}/>
        <button onClick={(e) => onClickStartPlace(e, currentItem)}>Get Start Location</button>
        <input value={currentItem.start?.name || ''} placeholder="Start Location" readOnly />
        <button onClick={(e) => onClickEndPlace(e, currentItem)}>Get End Location</button>
        <input value={currentItem.end?.name || ''} placeholder="End Location" readOnly />
        <input value={currentItem.distance || 0} readOnly/>
        <button onClick={(e) => onClickClearCurrent(e, currentItem)}>Clear</button>
      </div>
    </div>
  );
}
